...
int pesquisaProduto(Produto *produto, int *cont);
...
int pesquisaProduto(Produto *produto, int *cont){
    ...
    do{
	for(x=0; x<*cont; x++){
        tiraNProduto(produto,&contador);
		if(strcmp(nome, produto[x].cNome) == 0){
			printf("\nValor do produto: %.2f",produto[x].fValor);
			printf("\nDigite quantas unidades serao vendidas: ");
			scanf("%d",&unidade);
			total=produto[x].fValor*unidade;
			printf("\nValor total da compra: R$ %.2f",total);
			printf("\nDeseja adicionar mais um produto na venda?(s/n)");
			scanf("%c",&escolha);
		}
	}
}while(escolha!='n');
